...
let state = {
    ...
    settings: {
        owners: [
            { name: 'Administrator', username: 'superadmin' },
            { name: 'Pemilik', username: 'owner' },
            { name: 'Manajer', username: 'manager' }
        ],
        theme: 'pastel-blue',
        access: {
...
function showPage(pageId) {
    ...
    // Navbar visibility
    if (pageId === 'login' || pageId === 'intro' || pageId === 'test' || pageId === 'thankyou') {
...
function canManageShareSessions() {
    const isElectron = !!(window.electronAPI && window.electronAPI.createShareSession);
    const isWeb = !window.electronAPI;
    return (isElectron || isWeb) && !!state.userRole && !isGuestUser();
}
...
DOM.login.btn.addEventListener('click', () => {
    ...
    let role = null;
    if (user === 'superadmin') role = 'superadmin';
    else if (user === 'owner') role = 'owner';
    else if (user === 'manager') role = 'manager';
    else if (user === 'tamu') role = 'tamu';

    if (role) {
        state.userRole = role;
        DOM.login.error.classList.add('hidden');
        // Lookup display name and photo from owners list
        const ownerEntry = (state.settings.owners || []).find(o => o.username === user);
...
function renderShareSessions() {
    ...
    sessions.forEach(session => {
        ...
        item.querySelector('[data-action="close"]').addEventListener('click', async () => {
            let result;
            if (window.electronAPI?.closeShareSession) {
                result = await window.electronAPI.closeShareSession(session.id);
            } else {
                try {
                    const res = await fetch(`/api/share/sessions/${encodeURIComponent(session.id)}`, {
                        method: 'DELETE'
                    });
...
async function refreshShareSessions() {
    if (!canManageShareSessions()) return;
    let result;
    if (window.electronAPI?.getShareSessions) {
        result = await window.electronAPI.getShareSessions();
        state.shareSessions = result?.sessions || [];
    } else {
        try {
            const res = await fetch('/api/share/sessions');
            const data = await res.json();
            state.shareSessions = data?.sessions || [];
        } catch (err) {
            console.error('Gagal mengambil sesi tamu:', err);
        }
    }
    renderShareSessions();
}
...
async function createShareSession() {
    if (!canManageShareSessions()) return;
    let result;
    if (window.electronAPI?.createShareSession) {
        result = await window.electronAPI.createShareSession({
...
function saveHistory() {
    if (window.electronAPI) {
        ...
        if (!isSharedGuestMode()) {
            ...
            }).catch(err => console.error('Gagal menyimpan history ke server:', err));
        }
    }
}
...
function saveSettingsToDisk() {
    if (window.electronAPI && window.electronAPI.saveSettings) {
        ...
        }).catch(err => console.error('Gagal menyimpan settings ke server:', err));
    }
}
...
function applyAccessControl() {
    ...
    // Apply theme
    applyTheme(state.settings.theme || 'pastel-blue');
}
...
document.querySelectorAll('.settings-tab').forEach(tab => {
    tab.addEventListener('click', () => {
        ...
        const panelId = 'settings-panel-' + tab.dataset.tab;
...
function renderOwners() {
    ...
    owners.forEach((owner, idx) => {
        ...
        nameInput.placeholder = 'Nama Pemilik';
        ...
        rolesList.forEach(r => {
            ...
            if (owner.username === r) opt.selected = true;
...
DOM.settings.addOwnerBtn.addEventListener('click', () => {
    ...
    state.settings.owners.push({ name: '', username: 'tamu', photo: '' });
    renderOwners();
});
...
function renderThemes() {
    ...
    const currentTheme = state.settings.theme || 'pastel-blue';
...
function renderAccessTable() {
    ...
    featuresList.forEach(feat => {
        ...
        rolesList.forEach(role => {
            ...
            cb.type = 'checkbox';
            cb.checked = access[role] ? (access[role][feat.id] !== false) : true;
            
            if (role === 'superadmin' && feat.id === 'pengaturan') {
                cb.disabled = true;
            }
